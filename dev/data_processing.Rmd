...
# Update the BAM file paths
colData(ods)$RNA_BAM_FILE <- file.path(
  new_base_path,
  basename(colData(ods)$RNA_BAM_FILE)
)

colData(fds)$RNA_BAM_FILE <- file.path(
  new_base_path,
  basename(colData(fds)$RNA_BAM_FILE)
)

# Reassign proper BamFile objects
fds_bam_paths <- colData(fds)$RNA_BAM_FILE
colData(fds)$bamFile <- BamFileList(fds_bam_paths)
ods_bam_paths <- colData(ods)$RNA_BAM_FILE
colData(ods)$bamFile <- BamFileList(ods_bam_paths)
